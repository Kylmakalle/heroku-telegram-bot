...
def get_first_ability(player):
    ...
    if player.chat_id == 197216910:
        callback_button1 = types. \
            InlineKeyboardButton(text=special_abilities.WeaponMaster.name, callback_data=str('a' + str(special_abilities.abilities.index(special_abilities.WeaponMaster))))
        callback_button2 = types. \
            InlineKeyboardButton(text='Инфо', callback_data=str(str('i' + str(special_abilities.abilities.index(special_abilities.WeaponMaster)))))
        keyboard.add(callback_button1, callback_button2)
    bot.send_message(
...
def create_keyboard(buttons_list, row_length):
    markup = types.InlineKeyboardMarkup()
    for i in range(0, len(buttons_list), row_length):
        buttons = buttons_list[i:i + row_length]
        markup.row(*buttons)
    return markup
