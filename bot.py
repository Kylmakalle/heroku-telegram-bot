        ...
        if game.gamestate == game.gamestates[0] :
            ...
            if call.data[0] == 'i'and len(call.data) < 4:
                ...
                if actor in game.fight.playerpool:
                    ...
                    elif call.data[0:7] == 'execute':
                        print(actor.name + ' убивает')
                        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                              text="Цель выбрана: " + utils.actor_from_id(call.data[7:], game).name)
                        actor.target = utils.actor_from_id(call.data[7:], game)
                        actor.turn = 'execute'
                        actor.fight.playerpool.remove(actor)
